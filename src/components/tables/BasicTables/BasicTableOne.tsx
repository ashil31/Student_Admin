...
export default function StudentsRankingTable() {
  ...
  const classes = useMemo(
    () => ["All", ...Array.from(new Set(students.map((s) => s.className)))],
    []
  );
  const months = useMemo(
    () => [
      "All",
      ...Array.from(new Set(students.map((s) => s.month).filter(Boolean))),
    ],
    []
  );
  ...
  // Find top 3 students for podium
  ...
  return (
    <>
      {/* Champions Podium - OUTSIDE table container */}
        ...
        {secondPlace && (
            <div className="flex flex-col items-center">
              ...
              <div className="w-20 h-32 bg-gray-300 dark:bg-gray-700 rounded-t-lg flex flex-col items-center justify-center relative shadow-md">
                ...
                {/* <TrophyIcon className="w-6 h-6 absolute top-2 right-2 text-gray-500" /> */}
              </div>
        ...
        {firstPlace && (
          <div className="flex flex-col items-center">
            ...
            <div className="w-24 h-40 bg-yellow-400 dark:bg-yellow-600 rounded-t-lg flex flex-col items-center justify-center relative shadow-lg border-4 border-yellow-300 dark:border-yellow-500">
              ...
              {/* <TrophyIcon className="w-8 h-8 absolute top-2 right-2 text-yellow-700 dark:text-yellow-300" /> */}
            </div>
        ...
        {thirdPlace && (
          <div className="flex flex-col items-center">
            ...
            <div className="w-16 h-28 bg-orange-300 dark:bg-orange-600 rounded-t-lg flex flex-col items-center justify-center relative shadow-md">
              ...
              {/* <TrophyIcon className="w-5 h-5 absolute top-2 right-2 text-orange-600 dark:text-orange-300" /> */}
            </div>
          ...
          <TableBody>
            {filteredStudents.length === 0 ? (
              ...
              filteredStudents.map((student) => {
                ...
                if (student.rank === 1) {
                  rowClass =
                    "bg-yellow-50 dark:bg-yellow-900/20 rounded-lg shadow-sm hover:shadow-lg transition-shadow duration-300";
                } else if (student.rank === 2) {
                  rowClass =
                    "bg-gray-50 dark:bg-gray-800/20 rounded-lg shadow-sm hover:shadow-lg transition-shadow duration-300";
                } else if (student.rank === 3) {
                  rowClass =
                    "bg-orange-50 dark:bg-orange-900/20 rounded-lg shadow-sm hover:shadow-lg transition-shadow duration-300";
                } else {
                  rowClass =
                    "bg-white dark:bg-gray-900 rounded-lg shadow-sm hover:shadow-lg transition-shadow duration-300";
                }
                ...
                return (
                  <TableRow
                    key={student.rank}
                    className={rowClass}
                    style={{ borderBottom: "12px solid transparent" }}
                  >
                    <TableCell
                      className="px-5 py-5 sm:px-6 text-start dark:text-white/90 flex items-center gap-2"
                      style={{ border: "none" }}
                    >
                      {student.rank}
                      {(student.rank === 1 ||
                        ...
                        student.rank === 3) && (
                        <div
                          className={`flex items-center gap-1 font-semibold text-sm ml-2 ${
                            student.rank === 1
                    ...
                    <TableCell
                      className="px-5 py-5 sm:px-6 text-start"
                      style={{ border: "none" }}
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="font-semibold text-gray-900 dark:text-white/90 tracking-tight">
                            {student.name}
                          </p>
                          <p className="text-xs text-gray-500 dark:text-gray-400">
                            {student.className}
                    ...
                    <TableCell
                      className="px-5 py-5 sm:px-6 text-start font-semibold text-gray-700 dark:text-gray-300"
                      style={{ border: "none" }}
                    >
                      🔥 {student.streak} days
                    </TableCell>
                    ...
                    <TableCell
                      className="px-5 py-5 sm:px-6 text-start font-semibold text-gray-900 dark:text-white"
                      style={{ border: "none" }}
                    >
                      {student.points} pts
                    </TableCell>
                    ...
                    <TableCell
                      className="px-5 py-5 sm:px-6 text-center"
                      style={{ border: "none" }}
                    >
                      <Button
                        size="sm"
                        variant="outline"
                        className="transition-colors duration-200 hover:bg-gray-100 dark:hover:bg-white/10"
                      >
                        View Profile
                      </Button>
                    </TableCell>
